...
def tuple_alignment(sequences, gap_symbols='-', frame = 0, tuple_length = 3,
    ...
    frame_pattern = '(?:(?:^' + f'[^{gap_symbols}]'.join([f'[{gap_symbols}]*' for i in range(frame_comp+1)]) + f')|[{gap_symbols}]*)' + r'\K'
...
def import_augustus_training_file(paths, undersample_neg_by_factor = 1., alphabet=['a', 'c', 'g', 't'], reference_clades = None, 
    ...
    for p in range(len(paths)):
        ...
        with (gzip.open(path, 'rt') if path.endswith('.gz') else open(path, 'r')) as f:
            ...
            slice_pattern = re.compile(r'^[0-9]+\t[acgt\-]+$')
...
def parse_text_MSA(text_MSA, clades, use_codons=True, margin_width=0, num_positions = None, 
                   ...
                   sliding_window = False, ebony = False, output_all_species = False):
    """
    ...
    elif isinstance(text_MSA, dict):
        seq_msas = []
        if text_MSA: # if dict isnt empty
            seq_msas = [text_MSA]
    else:
    ...
    for seq_msa in seq_msas:
        ...
        frame = seq_msa["frame"]
        auxdata = None
        if "type" in seq_msa:  # ebony prediction, augustus prediction
            auxdata = {"seqname" : seq_msa["seqname"],
                       "type" : seq_msa["type"],
                       "coords" : seq_msa["coords"],
                       "plus_strand" : seq_msa["plus_strand"]}
            if "label" in seq_msa:  # augustus prediction
                auxdata.update({"label" : seq_msa["label"]})
        ...
        elif "seqname" in seq_msa and not output_all_species : # for BioPython MSAs from MAF
            auxdata = {"seqname" : seq_msa["seqname"],
                       "chrPos" : seq_msa["chrPos"],
                       "numSites" : seq_msa["numSites"],
                       "codon_strand" : seq_msa["codon_strand"]}
        elif "seqs" in seq_msa and output_all_species: # for BioPython MSAs from MAF
...
def parse_augustus_seqs(augfile: str):
    ...
    msa = {}
    species = {}
    slice_pattern = re.compile(r'^[0-9]+\t[acgtn\-]+$')
    ...
    with (gzip.open(augfile, 'rt') if (os.path.isfile(augfile) and augfile.endswith('.gz')) \
        ...
        while line:
            ...
            # parse the lines generated by GeneMSA::getMsa
            elif slice_pattern.match(line):
                slice_data = line.split("\t")
                s = slice_data[0]  # species id
                sequence = slice_data[1][:-1]
                if s not in species:
                    sys.exit(f"Error: species index {s} out of bounds in line\n{line}")
                msa['species'].append(species[s])
                msa['seqs'].append(sequence)
                    
            # retrieve the species
            elif line.startswith("species "): 
                _, sid, sname = line.split()
                if sid in species and species[sid] != sname:
                    sys.exit(f"Error: species index {s} is defined multiple times with species {species[sid]} and {sname}")
                species[sid] = sname
