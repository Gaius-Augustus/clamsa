...
def num_leaves(forest):
    '''
        Given a list of Newick files/strings gather the number of leaves occuring in this forest

        Args:
            forest (list(str)): List of paths to the considered Newick tree files or Newick strings
...
@tf.function
def parse_tfrecord_entry(entry, num_leaves, alphabet_size, sitewise = False):
    ...
    for split in wanted_splits:
        ...
        if split.interweave_models != None:
            if split.interweave_models == True:
                ...
                split_ds = tf.data.Dataset.sample_from_datasets(list(split_ds.values()), weights = split.interweave_models, seed = seed) # percentages of neg and pos
